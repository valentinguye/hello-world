...
rm(heilmayr_desa, heilmayr_desa_sf)
###########################################################################################################
...
mills_coord <- st_geometry(heilmayr_desa_prj)
...
# chaue element de la list-colonne origins est un integer de length différent, selon le nombre de 
...
# ou k correspond à l'une des geometries output de st_intersection, soit un polygon sans overlap, soit 
# avec 1, 2, 3 .. overlaps. 
# autrement dit les weights peuvent être 1/length(.)

#pour chaque CA i
# il faut réussir à sélectionner tous les polygons qui ont "i" en origins. 
# puis grouper ceux qui ont le même nombre d'overlaps. ou meme pas d'ailleurs en fait
# on peut juste "décomposer" sha_n en sha_n_1 sha_n_2 etc. i.e. les proportions du buffer total
# des différentes parties qui sont partagées par n autres. 

# une manière de sélectioner ce serait, pour chaque CA, de faire la liste des CAs qui l'intersectent. 
# et faire st_intersection parmi cette liste. 
# l'output i devrait toujours avoir 1 dans l'origins, 
# NOP il y a aussi toutes les intersections entre les "autres" "en-dehors" le CA d'intérêt. 
...
### NOTE  
# LES POLYGONS CORRESPONDANT SONT CEUX DONT CES NUMÉROS SONT LES INDICES *DANS exind[[km/10]][[t]][existing,]*
# i.e. les valeurs du sparse predicate sont les positions
# DANS Y (donc 1:488 pour t < 2004 par ex.) des polygons qui intersectent. 
# donc quand on utilise ces positions pour filtrer dans l'ensemble des mills on sélectionne d'autres poly
# cela vient de la manière dont st_intersects works.
