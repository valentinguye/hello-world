...
** noto
import excel "noto_done.xls", sheet("Sheet1") firstrow clear
...
save "noto_done.dta", replace 
*that's 102 mills with coordinates + 1 (AGROINDO INDAH PERKASA) with name but no coordinates. 

** pre2011
import excel "mills_to_georeference_pre2011_done.xlsx", sheet("Mills to georef") firstrow clear
rename latitude lat
...
save "mills_to_georeference_pre2011_done.dta", replace 
...
**post2010    
import excel "mills_to_georeference_post2010_done.xlsx", sheet("mills to georef") firstrow clear
sort firm_id year
rename latitude lat
rename longitude lon
drop if mi(firm_id)
...
save "mills_to_georeference_post2010_done.dta", replace 
...
** Heilmayr mill list 
import excel "C:\Users\guyv\ownCloud\opalval\build\input\traseMills_capEstyear.xlsx", sheet("traseMills_capEstyear") firstrow clear
...
codebook trase_code
save "C:\Users\guyv\ownCloud\opalval\build\input\traseMills_capEstyear_selected.dta", replace 

*merge them 
use "C:\Users\guyv\ownCloud\opalval\build\output\IBS_1998_cleaned.dta", clear
sort firm_id year

merge m:1 firm_id using "oto.dta", generate(merge_oto) keepusing(parent_co mill_name est_year lat lon) update

merge m:1 firm_id using "noto_done.dta", generate(merge_noto) keepusing(parent_co mill_name est_year lat lon) update

merge m:1 firm_id using "mills_to_georeference_pre2011_done.dta", generate(merge_pre2011) keepusing(mill_name lat lon) update
...
merge m:1 lat lon using "C:\Users\guyv\ownCloud\opalval\build\input\traseMills_capEstyear_selected.dta", generate(merge_heilmayr) keepusing(trase_code parent_co est_year) update 
drop if merge_heilmayr == 2

keep if !mi(lat)
keep firm_id year trase_code mill_name lat lon 
duplicates drop firm_id, force
save "C:\Users\guyv\ownCloud\opalval\build\temp\mill_geolocalization\IBS_mills_geolocalized.dta", replace
